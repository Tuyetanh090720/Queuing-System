
    <div id="layoutSidenav">
        <div id="layoutSidenav_nav">
            <nav class="sb-sidenav accordion sb-sidenav-dark" id="sidenavAccordion">
                <div class="sb-sidenav-menu">
                <div class="nav">
                    <a class="logo-header" href="{{route('admins.')}}">
                        <img src="{{asset('assets/admins/img/Logo alta.png')}}" alt="">
                    </a>
                    <a class="nav-link collapsed active" href="#" data-toggle="collapse" data-target="#collapse1" aria-expanded="false" aria-controls="collapse1">
                        <div class="sb-nav-link-icon"><i class="fa fa-window-restore"></i></div>
                        Dashboard
                    </a>
                    <a class="nav-link collapsed" href="#" data-toggle="collapse" data-target="#collapse2" aria-expanded="false" aria-controls="collapse2">
                        <div class="sb-nav-link-icon"><i class="fa fa-desktop"></i></div>
                        Thiết bị
                    </a>
                    <a class="nav-link collapsed" href="#" data-toggle="collapse" data-target="#collapse3" aria-expanded="false" aria-controls="collapse3">
                        <div class="sb-nav-link-icon"><i class="fa fa-comments"></i></div>
                        Dịch vụ
                    </a>
                    <a class="nav-link collapsed" href="#" data-toggle="collapse" data-target="#collapse4" aria-expanded="false" aria-controls="collapse4">
                        <div class="sb-nav-link-icon"><i class="fa fa-layer-group"></i></div>
                        Cấp số
                    </a>
                    <a class="nav-link collapsed" href="#" data-toggle="collapse" data-target="#collapse5" aria-expanded="false" aria-controls="collapse4">
                        <div class="sb-nav-link-icon"><i class="fa fa-chart-column"></i></div>
                        Báo cáo
                    </a>
                    <a class="nav-link collapsed" href="#" data-toggle="collapse" data-target="#collapse6" aria-expanded="false" aria-controls="collapse4">
                        <div class="sb-nav-link-icon"><i class="fa fa-gear"></i></div>
                        Cài đặt hệ thống
                        <div class="sb-sidenav-collapse-arrow"><i class="fa fa-ellipsis-h"></i></div>
                    </a>
                    <div class="collapse" id="collapse6" aria-labelledby="headingOne" data-parent="#sidenavAccordion">
                        <nav class="sb-sidenav-menu-nested nav">
                            <a class="nav-link" href="">Quản lý vai trò</a>
                            <a class="nav-link" href="">Quản lý tài khoản</a>
                            <a class="nav-link" href="">Nhật ký người dùng</a>
                        </nav>
                    </div>
                </div>
            </div>
            <div class="sb-sidenav-footer">
                <div><i class="fa fa-arrow-right-from-bracket"></i>Đăng xuất</div>
            </div>
            </nav>
        </div>
        <div id="layoutSidenav_content">


